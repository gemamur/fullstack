...
function saveArtist(req,res){
  ...
  artist.image = 'noimage.gif';
  artist.owner = params.owner;
  ...
  Artist.findByIdAndRemove(artistId, (err, artistRemoved)=>{
    if(err){
      ...
      if(!artistRemoved){
        ...
        Albumdisc.find({artist: artistRemoved._id}).remove((err, albumdiscRemoved)=>{
          if(err){
...
function buscarArtistas(req, res){
  ...
  Artist.find({name: {$regex : new RegExp (texto, "i")}}).sort('name').exec((err, artists)=>{
    if(err){
